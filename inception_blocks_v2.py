...
def inception_block_1a(X):
    ...
    # Using channels_last for all Conv2D layers
...
def inception_block_1c(X):
    ...
    X_5x5 = fr_utils.conv2d_bn(X,
                               ...
                               cv2_filter=(5, 5),
                               cv2_strides=(2, 2),
                               padding=(2, 2))  # Same assumption as above

    X_pool = MaxPooling2D(pool_size=3, strides=2, data_format='channels_last')(X)
    X_pool = ZeroPadding2D(padding=((0, 1), (0, 1)), data_format='channels_last')(X_pool)

    inception = concatenate([X_3x3, X_5x5, X_pool], axis=3)  # Changed axis to 3 for channels_last
...
def faceRecoModel(input_shape):
    ...
    # X = Lambda(lambda x: tf.linalg.normalize(x, axis=3)[0],output)(X)
    # Assuming `X` is a tensor of shape (batch_size, height, width, channels)
    X = Lambda(lambda x: tf.linalg.normalize(x, axis=1)[0], output_shape=lambda inp_shp: inp_shp)(X)
