...
from tensorflow.keras.layers import Conv2D, ZeroPadding2D, Activation, Input, BatchNormalization, MaxPooling2D, AveragePooling2D, concatenate
...
def img_to_encoding(image_path, model):
    ...
    img = cv2.resize(img, (96, 96))  # Resize to 96x96, assuming that's the expected input size
    img = img.astype('float32') / 255.0  # Normalize pixel values to [0, 1]
    img = np.expand_dims(img, axis=0)  # Add batch dimension
    embedding = model.predict(img)  # Get the encoding
    return embedding
